---
title: "Random Forest Model Results"
date: last-modified
---

```{r}
#| label: setup
#| include: false


knitr::opts_chunk$set(echo = T)
options(tidyverse.quiet = T)

library(targets)
tar_source('R')

library(MetBrewer)
```


# Land Surface Temperature 


## City Scale


```{r}
#| label: city-data
#| echo: false
#| eval: true

tar_load(cities_lst_full)

```


After prepping dataset, we check for correlations across variables :
```{r}
#| label: city-corr
#| echo: true
#| eval: true

cities_lst_full %>%
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()

```


There are a lot of correlations in the variables at this scale, we can use a `step_corr()` function to automate their removal, or do it by hand. We will proceed with the `step_corr()` function.

Now, we start to specify our model: 

```{r}
#| label: city-workflow
#| echo: true
#| eval: true

set.seed(123)
# strata ensures that the random sampling is conducted within the stratification variable
# resamples have equivalent proportions of the original dataset
df_split <- initial_split(cities_lst_full %>% rename(value = mean_temp), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)


# this is the model, everything in the dataset is explanatory except legal status and ID
mod_rec <- recipe(value ~ ., data = df_train) %>%
  # normalize all continuous vars
  step_normalize(all_numeric_predictors(), -doy) %>% 
  # converts factors (nominal) into numeric binary model terms
  step_dummy(all_nominal(), -all_outcomes()) %>%
  # set correlation threshold to 0.8
  step_corr(threshold = 0.8) %>% 
  # remove variables that have near zero variation
  step_nzv()


# tune hyperparameters mtry and min_n
tune_spec <- rand_forest(
  mtry = tune(), # number of predictors that will be randomly sampled at each split
  trees = 1000,
  min_n = tune() # min number of data points in a node required for the node to split further
) %>%
  # regression is trying to predict/fit numeric values
  set_mode("regression") %>%
  # type of computational engine -> dependent on model type
  # ranger is default and allows tuning of hyperparameters
  set_engine("ranger")

# put everything together in a workflow
tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)
```

After this, we want to tune our hyperparameters so that we select the ones with the best performance 

```{r}
#| label: city-tuning
#| echo: true
#| eval: true
#| message: false

set.seed(234)

# random splits of the data into V groups of roughly equal size
# using boostrap because small dataset
mod_folds <- bootstraps(df_train)

# need to assess across many models - use parallel processing to use 20 grid points
future::plan(strategy = 'multisession')

set.seed(345)
# computes performance metrics for tuning parameters (set in tune_wf) for resamples (trees_folds)
tune_res <- tune_grid(
  tune_wf,
  resamples = mod_folds,
  # data frame of tuning combinations with 25 candidate parameter sets
  grid = 15,
  metrics = metric_set(mae, rsq))
```

Assess the performance of different hyperparameter values

```{r}
#| label: city-model-selection
#| echo: true
#| eval: true


autoplot(tune_res)


# select best values for hyperparameters and fit model
best_auc <- select_best(tune_res, metric = "mae")

```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: city-fit
#| echo: true
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_auc$mtry, min_n = best_auc$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]


```

Now we can assess the performance of the final model. We want metrics to stay consistent across testing and training data. 


```{r}
#| label: city-performance
#| echo: true
#| eval: true

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


```

The city model is not great at predicting data. This makes sense, it is a small dataset with a lot of autocorrelation. The test data performs better than the training data, though ideally these metrics would be similar across groups. This model is weak.

The worst predictions seem to be at very low and very high mean temperatures.

Let's see what the most important variables in our model are

```{r}
#| label: city-vip
#| echo: true
#| eval: true

vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```

We only see 1 street tree variable in the 10 most important variables at this scale. What are the relationships for each city of these top variables? 

```{r}
#| label: city-pdp
#| echo: true
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)

#supp.labs <- c("Mean NDVI", "Day of Year", "Mean NDBI", "Visible Minorities (%)", "Road Density (km/km2)", "Tree Stem Density (km2/km2)", "City Area (km2)", "Std Dev Building Height (m)", "Duplex (% residents)", "Recent Immigrants (%)")
#names(supp.labs) <- vars

as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  #geom_point(data = df_train, aes(x = stemdens, y = mean_temp, colour = city)) + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at City Scale",
    y = "Predicted mean temp",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```

## Neighbourhood Scale 

The code is the same so we will only be looking at table and figure outputs. 


```{r}
#| label: nhood-data
#| echo: false
#| eval: true

tar_load(neighbourhoods_lst_full)

```


After prepping dataset, we check for correlations across variables :
```{r}
#| label: nhood-corr
#| echo: false
#| eval: true

neighbourhoods_lst_full %>%
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()

```


There are a couple strong correlations in the variables at this scale. We will proceed with the `step_corr()` function. We will use cross-fold validation instead of bootstrapping at this scale, because we have enough samples.

```{r}
#| label: nhood-workflow
#| echo: false
#| eval: true


set.seed(123)
# strata ensures that the random sampling is conducted within the stratification variable
# resamples have equivalent proportions of the original dataset
df_split <- initial_split(neighbourhoods_lst_full %>% rename(value = mean_temp), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)


# this is the model, everything in the dataset is explanatory except legal status and ID
mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors()) %>% 
  # converts factors (nominal) into numeric binary model terms
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()


# tune hyperparameters mtry and min_n
tune_spec <- rand_forest(
  mtry = tune(), # number of predictors that will be randomly sampled at each split
  trees = 1000,
  min_n = tune() # min number of data points in a node required for the node to split further
) %>%
  # mode options are unknown, regression, classification, censored regression
  # classification mode is trying to predict differences in categories
  # regression is trying to predict/fit numeric values
  set_mode("regression") %>%
  # type of computational engine -> dependent on model type
  # random forest models have ranger, randomForest, and spark as options
  # ranger is default and allows tuning of hyperparameters
  set_engine("ranger")

# put everything together in a workflow
tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)

tune_res <- tar_read(neighbourhoods_temp_tune)
```

Assess the performance of different hyperparameter values

```{r}
#| label: nhood-model-selection
#| echo: false
#| eval: true


autoplot(tune_res)


# select best values for hyperparameters and fit model
best_auc <- select_best(tune_res, metric = "mae")
```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: nhood-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_auc$mtry, min_n = best_auc$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))


last_rf_model <- last_rf_fit$.workflow[[1]]


# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```

EXPLANATION 

```{r}
#| label: nhood-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)

#supp.labs <- c("Mean NDVI", "Day of Year", "Mean NDBI", "Visible Minorities (%)", "Road Density (km/km2)", "Tree Stem Density (km2/km2)", "City Area (km2)", "Std Dev Building Height (m)", "Duplex (% residents)", "Recent Immigrants (%)")
#names(supp.labs) <- vars

as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  #geom_point(data = df_train, aes(x = stemdens, y = mean_temp, colour = city)) + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Neighbourhood Scale",
    y = "Predicted mean temp",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```



## Street Scale 

```{r}
#| label: street-data
#| echo: false
#| eval: true

tar_load(roads_lst_full)

```


After prepping dataset, we check for correlations across variables :
```{r}
#| label: street-corr
#| echo: false
#| eval: true

roads_lst_full %>%
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
# strata ensures that the random sampling is conducted within the stratification variable
# resamples have equivalent proportions of the original dataset
df_split <- initial_split(roads_lst_full %>% rename(value = mean_temp), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)


# this is the model, everything in the dataset is explanatory except legal status and ID
mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors()) %>% 
  # converts factors (nominal) into numeric binary model terms
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()


# tune hyperparameters mtry and min_n
tune_spec <- rand_forest(
  mtry = tune(), # number of predictors that will be randomly sampled at each split
  trees = 1000,
  min_n = tune() # min number of data points in a node required for the node to split further
) %>%
  # mode options are unknown, regression, classification, censored regression
  # classification mode is trying to predict differences in categories
  # regression is trying to predict/fit numeric values
  set_mode("regression") %>%
  # type of computational engine -> dependent on model type
  # random forest models have ranger, randomForest, and spark as options
  # ranger is default and allows tuning of hyperparameters
  set_engine("ranger")

# put everything together in a workflow
tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)

tune_res <- tar_read(streets_temp_tune)


autoplot(tune_res)


# select best values for hyperparameters and fit model
best_auc <- select_best(tune_res, metric = "mae")
```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: street-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_auc$mtry, min_n = best_auc$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))


last_rf_model <- last_rf_fit$.workflow[[1]]


# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```

EXPLANATION 

```{r}
#| label: street-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)

#supp.labs <- c("Mean NDVI", "Day of Year", "Mean NDBI", "Visible Minorities (%)", "Road Density (km/km2)", "Tree Stem Density (km2/km2)", "City Area (km2)", "Std Dev Building Height (m)", "Duplex (% residents)", "Recent Immigrants (%)")
#names(supp.labs) <- vars

as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  #geom_point(data = df_train, aes(x = stemdens, y = mean_temp, colour = city)) + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Street Scale",
    y = "Predicted mean temp",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```


# Air Pollution

## UV 

### City 

```{r}
#| label: city-uv-data
#| echo: false
#| eval: true

tar_load(cities_pollution_full)

cities_pollution_full %>%
  filter(variable == 'mean_UV_city') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(cities_pollution_full %>% filter(variable == 'mean_UV_city') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(cities_uv_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")

```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: city-uv-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)

# vip

vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: city-uv-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at City Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```

### Neighbourhood 


```{r}
#| label: nhood-uv-data
#| echo: false
#| eval: true

tar_load(neighbourhoods_pollution_full)

neighbourhoods_pollution_full %>%
  filter(variable == 'mean_UV_neighbourhood') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(neighbourhoods_pollution_full %>% filter(variable == 'mean_UV_neighbourhood') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(neighbourhoods_uv_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")

```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: nhood-uv-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)

# vip

vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: nhood-uv-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Neighbourhood Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```


## CO 

### City 

```{r}
#| label: city-co-data
#| echo: false
#| eval: true

cities_pollution_full %>%
  filter(variable == 'mean_CO_city') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(cities_pollution_full %>% filter(variable == 'mean_CO_city') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(cities_co_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")


```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: city-co-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: city-co-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable

vars <- vars[vars != 'city_Winnipeg']

pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at City Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```

### Neighbourhood 

```{r}
#| label: nhood-co-data
#| echo: false
#| eval: true

neighbourhoods_pollution_full %>%
  filter(variable == 'mean_CO_neighbourhood') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(neighbourhoods_pollution_full %>% filter(variable == 'mean_CO_neighbourhood') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(neighbourhoods_co_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")


```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: nhood-co-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: nhood-co-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable

vars <- vars[vars != 'city_Winnipeg']

pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Neighbourhood Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```


## NO2

### City 

```{r}
#| label: city-no2-data
#| echo: false
#| eval: true

cities_pollution_full %>%
  filter(variable == 'mean_NO2_city') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(cities_pollution_full %>% filter(variable == 'mean_NO2_city') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(cities_no2_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")



```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: city-no2-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: city-no2-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at City Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```

### Neighbourhood


```{r}
#| label: nhood-no2-data
#| echo: false
#| eval: true

neighbourhoods_pollution_full %>%
  filter(variable == 'mean_NO2_neighbourhood') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(neighbourhoods_pollution_full %>% filter(variable == 'mean_NO2_neighbourhood') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(neighbourhoods_no2_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")



```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: nhood-no2-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: nhood-no2-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Neighbourhood Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```


## SO2

### City 

```{r}
#| label: city-so2-data
#| echo: false
#| eval: true

cities_pollution_full %>%
  filter(variable == 'mean_SO2_city') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(cities_pollution_full %>% filter(variable == 'mean_SO2_city') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(cities_so2_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")


```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: city-so2-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: city-so2-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at City Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```

### Neighbourhood 


```{r}
#| label: nhood-so2-data
#| echo: false
#| eval: true

neighbourhoods_pollution_full %>%
  filter(variable == 'mean_SO2_neighbourhood') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(neighbourhoods_pollution_full %>% filter(variable == 'mean_SO2_neighbourhood') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(neighbourhoods_so2_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")


```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: nhood-so2-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: nhood-so2-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Neighbourhood Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```


## O3

### City

```{r}
#| label: city-o3-data
#| echo: false
#| eval: true

cities_pollution_full %>%
  filter(variable == 'mean_O3_city') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(cities_pollution_full %>% filter(variable == 'mean_O3_city') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(cities_o3_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")


```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: city-o3-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: city-o3-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at City Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```

### Neighbourhood 


```{r}
#| label: nhood-o3-data
#| echo: false
#| eval: true

neighbourhoods_pollution_full %>%
  filter(variable == 'mean_O3_neighbourhood') %>% 
  select_if(is.numeric) %>%
  cor(use = "complete.obs") %>%
  corrplot::corrplot()


set.seed(123)
df_split <- initial_split(neighbourhoods_pollution_full %>% filter(variable == 'mean_O3_neighbourhood') %>% select(-variable), strata = city)
df_train <- training(df_split)
df_test <- testing(df_split)

mod_rec <- recipe(value ~ ., data = df_train) %>%
  step_normalize(all_numeric_predictors(), -doy) %>% 
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_corr(threshold = 0.8) %>% 
  step_nzv()

tune_spec <- rand_forest(
  mtry = tune(), 
  trees = 1000,
  min_n = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")

tune_wf <- workflow() %>%
  add_recipe(mod_rec) %>%
  add_model(tune_spec)


cities_tune <- tar_read(neighbourhoods_o3_tune)


autoplot(cities_tune)


# select best values for hyperparameters and fit model
best_mae <- select_best(cities_tune, metric = "mae")


```

Update model to include variance importance scores and fit best model to test data

```{r}
#| label: nhood-o3-fit
#| echo: false
#| eval: true

cores <- parallel::detectCores()

last_mod <-
  rand_forest(mtry = best_mae$mtry, min_n = best_mae$min_n, trees = 1000) %>%
  set_mode("regression") %>%
  set_engine("ranger", num.threads = cores, importance = "impurity")


# the last workflow
last_workflow <-
  tune_wf %>%
  update_model(last_mod)

# the last fit
set.seed(345)
last_rf_fit <-
  last_workflow %>%
  last_fit(df_split,
           metrics = metric_set(mae, rsq))

last_rf_model <- last_rf_fit$.workflow[[1]]

# prediction vs actual values for final model
all_predictions <- bind_rows(
  augment(last_rf_model, new_data = df_train) %>% 
    mutate(type = "train"),
  augment(last_rf_model, new_data = df_test) %>% 
    mutate(type = "test")
)

# metrics between testing and training 

all_predictions %>% 
  group_by(type) %>% 
  metrics(value, .pred)

# model fit between testing and training 

all_predictions %>%
  ggplot(aes(value, .pred)) +
  geom_point() +
  geom_abline() +
  facet_wrap(~type)


vip <- last_rf_fit %>%
  extract_fit_parsnip() %>%
  vip(geom = "point") +
  theme_classic()

vip
```


```{r}
#| label: nhood-o3-pdp
#| echo: false
#| eval: true
#| out-width: 100%

model_explainer <- explain_tidymodels(
  last_rf_fit$.workflow[[1]],
  data = df_train,
  y = as.integer(df_train$value),
  verbose = FALSE
)

vars <- vip[["data"]]$Variable


pdp_time <- model_profile(
  model_explainer,
  variables = vars,
  N = NULL,
  groups = "city"
)


as_tibble(pdp_time$agr_profiles) %>%
  mutate(across(`_vname_`, ~factor(., levels=vars))) %>%
  ggplot(aes(`_x_`, `_yhat_`, color = `_groups_`)) +
  scale_color_met_d(name = 'Demuth') + 
  geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "10 Most Important Variables at Neighbourhood Scale",
    y = "Predicted UV",
    colour = "",
    x = ""
  ) + 
  facet_wrap(`_vname_` ~ ., scales = "free") + 
  theme_classic()

```
