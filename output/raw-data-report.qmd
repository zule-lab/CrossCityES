...
city_tree_vars <- cities_lst_full %>% select(c(city, nTrees, ba_per_m2, stemdens, SpeciesRichness, Shannon, nFG, mean_dbh, sd_dbh)) %>% 
...
city_building_vars <- cities_lst_full %>% 
  select(c(city, centroid_den, area_den, prop_highway, prop_strts, road_dens, NDBI_mean_, NDBI_stdDev_, NDVI_mean_, NDVI_stdDev_)) %>% 
  pivot_longer(-city, names_to = 'variable')
...
city_census_vars <- cities_lst_full %>% 
  select(c(city, popdens, sidehop, aptfivp, semhoup, rowhoup, aptdupp, aptbuip, mvdwelp, medinc, lowinc, recimmp, indigp, visminp, edubacp)) %>% 
  pivot_longer(-city, names_to = 'variable')
...
neighbourhood_building_vars <- neighbourhoods_lst_full %>% 
  select(c(city, hood, centroid_den, area_den, prop_highway, prop_strts, road_dens, NDBI_mean_, NDBI_stdDev_, NDVI_mean_, NDVI_stdDev_)) %>% 
  pivot_longer(-c(city, hood), names_to = 'variable')
...
neighbourhood_census_vars <- neighbourhoods_lst_full %>% 
  select(c(city, hood, popdens, sidehop, aptfivp, semhoup, rowhoup, aptdupp, aptbuip, mvdwelp, medinc, lowinc, recimmp, indigp, visminp, edubacp)) %>% 
  pivot_longer(-c(city, hood), names_to = 'variable')

city_plot(neighbourhood_census_vars)
```
...
mean_dbh = drop_units(mean_dbh),
sd_dbh = drop_units(sd_dbh)) %>%
pivot_longer(-c(city, streetid), names_to = 'variable') 
...
street_building_vars <- roads_lst_full %>% 
  select(c(city, streetid, centroid_den, area_den, NDBI_mean_, NDBI_stdDev_, NDVI_mean_, NDVI_stdDev_)) %>% 
  pivot_longer(-c(city, streetid), names_to = 'variable')
...
street_census_vars <- roads_lst_full %>% 
  select(c(city, streetid, popdens, sidehop, aptfivp, semhoup, rowhoup, aptdupp, aptbuip, mvdwelp, medinc, lowinc, recimmp, indigp, visminp, edubacp)) %>% 
  pivot_longer(-c(city, streetid), names_to = 'variable')
